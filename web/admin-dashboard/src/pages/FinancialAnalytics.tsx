        ...
        <div className="space-y-6 animate-in fade-in zoom-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {(periodStart || periodEnd) && (
                        <Button variant="outline" onClick={() => { setPeriodStart(""); setPeriodEnd(""); }}>
            ...
            {selectedVendorId === "all" ? (
                <div className="flex items-center justify-center h-64 border-2 border-dashed rounded-xl border-gray-200">
                    <p className="text-gray-500">{t("financial.please_select_vendor", "Please select a vendor to view deep analytics.")}</p>
                </div>
            ) : isLoading ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    <Card className="hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-white to-gray-50 border-gray-200">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-gray-600">{t("financial.total_orders", "Total Orders")}</CardTitle>
                            <Activity className="h-5 w-5 text-indigo-500" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-gray-900">{analyticsData?.totalOrders?.toLocaleString() || 0}</div>
                        </CardContent>
                    </Card>

                    <Card className="hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-white to-gray-50 border-gray-200">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-gray-600">{t("financial.gross_revenue", "Gross Revenue (Subtotal)")}</CardTitle>
                            <DollarSign className="h-5 w-5 text-emerald-500" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-gray-900">${analyticsData?.totalRevenue?.toLocaleString() || "0.00"}</div>
                        </CardContent>
                    </Card>

                    <Card className="hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-indigo-50 to-blue-50 border-indigo-100">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-indigo-800">{t("financial.platform_commission", "Platform Commission")}</CardTitle>
                            <Wallet className="h-5 w-5 text-indigo-600" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-indigo-900">-${analyticsData?.platformCommission?.toLocaleString() || "0.00"}</div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-orange-50 to-red-50 border-orange-100 flex flex-col justify-between">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-red-800">{t("financial.delivery_fees", "Delivery Fees")}</CardTitle>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-red-700/80 mt-1">{analyticsData?.totalDeliveries || 0} {t("financial.deliveries_completed")}</p>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="md:col-span-2 shadow-md hover:shadow-xl transition-all duration-300 bg-gradient-to-tr from-emerald-500 to-teal-400 text-white border-0">
                        <CardHeader>
                            <CardTitle className="text-lg font-medium opacity-90">{t("financial.net_revenue", "Vendor Net Revenue")}</CardTitle>
                        </CardHeader>

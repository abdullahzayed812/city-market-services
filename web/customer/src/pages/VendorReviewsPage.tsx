    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          size={13}
          className={n <= stars ? 'fill-accent text-accent' : 'text-gray-200 fill-gray-200'}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="bg-white rounded-2xl shadow-card p-5 mb-5">
        <h2 className="font-bold text-text-primary mb-4">{vendor?.name}</h2>
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1 space-y-1.5">
            {[5, 4, 3, 2, 1].map((n) => {
              const count =
                summary?.data?.ratingDistribution?.[n] ?? 0;

...
import {
  Bell, CheckCheck, ChevronLeft, Package, AlertCircle, Info,
  Bike, ShoppingBag, CheckCircle, AlertTriangle, BellOff, Volume2, VolumeX,
} from 'lucide-react';
    ...
    <motion.div
      ...
      className={`flex items-center gap-3 px-4 py-3 rounded-2xl mb-4 ${
        permissionStatus === 'denied'
          ? 'bg-error-light border border-red-200'
          : 'bg-accent-xlight border border-amber-200'
      }`}
    >
      <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
        permissionStatus === 'denied' ? 'bg-error/10' : 'bg-accent/10'
      }`}>
        {permissionStatus === 'denied' ? (
          <BellOff size={15} className="text-error" />
        ) : (
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-text-muted">
          {permissionStatus === 'denied'
            ? 'Go to browser settings to allow CityMarket notifications.'
    ...
    <motion.div
      ...
      className={`flex gap-3 p-4 rounded-2xl transition-all cursor-pointer group ${
        n.isRead
          ...
          : 'bg-primary-xlight shadow-card border border-primary/10 hover:bg-primary/[0.08]'
      }`}
    >
      {/* Icon */}
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-bold mb-0.5 leading-snug ${
          n.isRead ? 'text-text-primary' : 'text-text-primary'
        }`}>
          {n.title}
        </p>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <button
          ...
          className={`w-9 h-9 flex items-center justify-center rounded-xl transition-colors ${
            soundEnabled
              ? 'bg-primary-xlight text-primary-darker hover:bg-primary-light'
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-text-muted max-w-xs">
            New order updates and alerts will appear here as they arrive.
          </p>
        </div>
      ) : (

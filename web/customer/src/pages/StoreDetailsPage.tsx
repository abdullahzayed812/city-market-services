...
import {
  Star, Clock, MapPin, ChevronRight, Package, ArrowLeft,
  ShoppingCart, ChevronLeft, Info,
} from 'lucide-react';
...
export default function StoreDetailsPage() {
  ...
  const { data: productsData, isLoading: productsLoading } = useQuery({
    ...
    queryFn: () =>
      CatalogService.getVendorProductsByVendor(vendorId!, 1, 20, activeCategory || undefined),
    enabled: !!vendorId,
  });
            ...
            <div className="flex-1">
              ...
              {ratingSummary?.data?.averageRating ? (
                <p className="text-xs text-text-muted">
                  {Number(ratingSummary.data.averageRating).toFixed(1)} stars ·{' '}
                  {ratingSummary.data.totalRatings || 0} reviews
                </p>
              ) : (
          ...
          {productsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-text-muted mt-1">
                This {activeCategory ? 'category' : 'store'} has no products available
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">

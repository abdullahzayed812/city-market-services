...
import { User, MapPin, Bell, Globe, LogOut, ChevronRight, Phone, Shield, HelpCircle, Edit2 } from "lucide-react";
    ...
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-6 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden mb-4">
        {menuItems.map((item, i) => (
          <div key={item.to}>
            <MenuItem {...item} />
            {i < menuItems.length - 1 && <div className="h-px bg-border-light mx-4" />}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden">
        <button onClick={handleSignOut} className="flex items-center gap-3 w-full px-4 py-3.5 hover:bg-error-light transition-colors">

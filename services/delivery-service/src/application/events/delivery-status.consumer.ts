...
export class DeliveryStatusConsumer implements EventSubscriber {
  ...
  async handle(event: BaseEvent): Promise<void> {
    ...
    const { customerOrderId, vendorOrderId, token } = event.payload; 
    ...
    try {
      if (newVendorOrderStatus && vendorOrderId) {
        await this.orderHttpClient.updateVendorOrderStatus(vendorOrderId, newVendorOrderStatus, token);
      ...
      if (newCustomerOrderStatus && customerOrderId) {
        await this.orderHttpClient.updateCustomerOrderStatus(customerOrderId, newCustomerOrderStatus, token);

...
export class DeliveryFeeCalculator {
   ...
   */
  static calculate(customerLat?: number, customerLng?: number, vendorLocations: { latitude?: number; longitude?: number }[] = []): number {
    // Fallback if no coordinates provided
    if (!customerLat || !customerLng || vendorLocations.length === 0) {
    ...
    for (const vendor of vendorLocations) {
      if (vendor.latitude && vendor.longitude) {
        const distance = this.haversineDistanceKm(customerLat, customerLng, vendor.latitude, vendor.longitude);
    ...
    if (maxDistance === 0 && vendorLocations.every((v) => !v.latitude)) {
      return this.DEFAULT_FALLBACK_FEE;
    }
